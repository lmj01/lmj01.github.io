...
<details>
<summary> diff log tag stash</summary>
...
从upstream上游fork时，对curRepo的仓库设定特定的目录，实现特定的接口

```shell
# 
git clone repo-url
git remote add upstream repo-upstream-url
# 每次合并上游分支时，要处理特定的目录
git fetch upstream
git merge upstream/main --no-commit
git checkout HEAD -- custom/   # 强制执行：无论上游发生了什么，把我的目录拿回来
git add custom/
git commit -m "Merge upstream and force keep custom dir"
```
