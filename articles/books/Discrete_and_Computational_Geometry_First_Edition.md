...
### 2.4 GIFT Warpping and graham scan

gift-wrapping algorithm礼品包装算法，又称Jarvis March贾维斯步进法，卷包裹算法，是计算机结婚用来求点集凸包Convex Hull的一种经典算法
时间复杂度O(nh)

![gift-warpping](/images/gift-wrapping.png)

Graham Scan格雷厄姆扫描法，是计算几何中求点集的凸包的经典高效算法，核心是极角排序+栈筛选。时间复杂度O(nlogn),优于礼品包装算法

![Graham Scan](/images/graham-scan.png)

### 2.5 Lower Bound

Let S be a point set in the plane. An algorithm that finds the hull points of S in the order of walking around the convex hull
cannot be faster than 
$$ O(n log n) $$ 
time. That is, 
$$ \Omega(n log n) $$ 
is a lower bound on the number of comparisons made by a sorting algorithm.
