...
javascript的超集，更加面向对象的编程语言，可以便宜为纯Javascript，带有类型的JavaScript
...
```typescript
// union
type TypeXXX = type1 | type2 | type3
// interface Typescript的interface比起C#或Java来说时有区别的，Typescript更加广泛，
interface SomePoint {
	x: number; 
	y: number;
}
...
- 条件类型 A extends B ? X : Y	类型层面的 if-else
- infer	在 extends 模式中"占位"并推断出某个类型

```typescript
// 提取参数元组， 内置也是这样的代码
type Parameters<F> = F extends (...args: infer P) => any ? P : never
// 例子
type P = Parameters<(a: string, b: number) => void> // P = [a: string, b: number]

// 提取返回值
type ReturnType<F> = F extends (...args: any) => infer R ? R : never
type R = ReturnType<() => string> // R = string

// 提取显示this
type ThisType<F> = F extends (this: infer T, ...args: any) => any ? T : never
type T = ThisType<(this: { name: string }, x: number) => void> // T = { name: string }
...
用来定义类型信息及接口规范，当使用扩展的JS库或插件API时，需要使用声明文件来描述库的类型。

比如编辑器需要给引用库的提示，就需要类型信息与接口。所以早期的Javascript 库时没有类型定义信息的，需要创建一个对应的d.ts文件，如three-js这样的库。
